...
def build_prompt(function_block, fullcode):
    full_prompt = (
        "generate commentary for the following method\n"
        "{block}\n"
    ...
    # replace "{file_name}" with file name
    full_prompt = full_prompt.replace("{full_code}", fullcode)
    # replace method stub with block
    full_prompt = full_prompt.replace("{block}", function_block)

    print("for debugging prompt:" + full_prompt)
    ...
    for dirpath, _, files in os.walk(source_dir):
        for file in files:
            ...
            for method_name, doc, block in methods:
                prompt = build_prompt(block, java_text)
                print(f"Sending method '{method_name}' from class '{class_name}' to model...")
                ai_response = run_model(prompt, model)
                results.append({
